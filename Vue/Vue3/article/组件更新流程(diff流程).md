在组件渲染的过程，本质上就是把各种类型的 vnode 渲染成真实 DOM。

我们也知道了组件是由模板、组件描述对象和数据构成的，数据的变化会影响组件的变化。组件的渲染过程中创建了一个`带副作用的渲染函数`，`当数据变化的时候就会执行这个渲染函数`来触发组件的更新。那么接下来，我们就具体分析一下组件的更新过程。



## 副作用渲染函数更新组件的过程

下面是这个带副作用渲染函数 setupRenderEffect 的实现，在渲染流程我们看的是 if 内的逻辑，这次我们重点关注 else 部分，也就是更新组件部分的逻辑：
...
首先是更新组件 vnode 节点，这里会有一个条件判断，判断组件实例中是否有新的组件 vnode（用 next 表示），有则更新组件 vnode，没有则将 next 指向之前的组件 vnode。

> 为什么需要判断，这里其实涉及一个组件更新策略的逻辑，我们稍后会讲。
...
const updateComponent = (n1, n2, optimized) => {
  ...
  if (shouldUpdateComponent(n1, n2, optimized)) {
    ...
    instance.next = n2
    // 子组件也可能因为数据变化被添加到更新队列里了，这里需要移除它们，以防止对一个子组件重复更新
...
可以看到，processComponent 主要通过执行 updateComponent 函数来更新子组件，updateComponent 函数在更新子组件的时候，会先执行 shouldUpdateComponent 函数，根据新旧子组件 vnode 来判断是否需要更新子组件。

这里你只需要知道，在 shouldUpdateComponent 函数的内部，主要是通过检测和对比组件 vnode 中的 `props`、`children`、`dirs`、`transiton` 等属性，来决定子组件是否需要更新。
...
如果 shouldUpdateComponent 返回 true ，那么在它的最后，先执行 invalidateJob（instance.update）`避免子组件由于自身数据变化导致的重复更新`，然后又执行了子组件的副作用渲染函数 instance.update 来 `主动触发子组件的更新` 。



再回到副作用渲染函数 instance.update() 中，有了前面的讲解，我们再看组件更新的这部分代码，就能很好地理解它的逻辑了

instance.update() 就是执行 setupRenderEffect 函数中的回调函数 componentUpdateFn

```ts
// packages/runtime-core/src/render.ts/baseCreateRenderer/setupRenderEffect/componentUpdateFn
...
updateComponentPreRender 函数：我们在更新组件的 DOM 前，需要先更新组件 vnode 节点信息，包括更改组件实例的 vnode 指针、更新 props 和更新插槽等一系列操作，因为组件在稍后执行 renderComponentRoot 时会重新渲染新的子树 vnode ，它依赖了更新后的组件 vnode 中的 props 和 slots 等数据。

所以，我们现在知道了一个组件重新渲染可能会有两种场景：
...
那么，这个子组件对应的新的组件 vnode（也就是新的子树vnode）是什么时候创建的呢？答案很简单，它是在父组件重新渲染的过程中，通过 renderComponentRoot 渲染子树 vnode 的时候生成。
...
所以，总结一下： processComponent 处理组件 vnode，本质上就是去判断子组件是否需要更新，如果需要则递归执行子组件的副作用渲染函数来更新，否则仅仅更新一些 vnode 的属性，并让子组件实例保留对组件 vnode 的引用，用于子组件自身数据变化引起组件重新渲染的时候，在渲染函数内部可以拿到新的组件 vnode。
...
其实这是很好理解的，因为一个 DOM 节点元素就是由它 `自身的一些属性(props)` 和 `子节点` 构成的。



首先是更新 props，这里的 patchProps 函数就是在更新 DOM 节点的 class、style、event 以及其它的一些 DOM 属性，这个过程不再深入分析了。
...
对于一个元素的子节点 vnode 可能会有三种情况：

1. 纯文本
2. vnode 数组
3. 空

那么根据排列组合对于新旧子节点来说就有 `九种情况`，我们可以通过三张图来表示。
...
新子节点数组相对于旧子节点数组的变化，无非是通过`更新`、`删除`、`添加`和`移动`节点来完成，而核心 diff 算法，就是在已知旧子节点的 DOM 结构、vnode 和新子节点的 vnode 情况下，以较低的成本完成子节点的更新为目的，求解生成新子节点 DOM 的系列操作。
